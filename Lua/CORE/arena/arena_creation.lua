...
local function update_arena(arena, forceupdate, moveplayer)
	...
	--if arena.isMoving or forceupdate then
		...
		arena.base.MoveTo(arena.currentx, arena.currenty)
		arena.walls.MoveTo(arena.center.absx, arena.center.absy )
...
function library.create_arena(self, x, y, w, h, r)
	...
	rawset(new_arena,"Remove", function()
		outer.remove()
		inner.remove()
		dummy.remove()
	...
	function shell.MoveRelative(bullet, ix, iy)
	--local px, py = Player.absx, Player.absy
